...
class ECDHEncryptionProvider(
        ...
        privateKeyString.onEach {
            privateKey = runCatching {
                ecdhProvider.privateKeyDecoder(
                    EC.Curve.P521
                ).decodeFromByteArray(EC.PrivateKey.Format.DER, Base64.decode(it))
            }.getOrNull()
            if (privateKey == null) {
                generateKeyPair()
            }
        ...
        publicKeyString.onEach {
            publicKey = runCatching {
                ecdhProvider.publicKeyDecoder(
                    EC.Curve.P521
                ).decodeFromByteArray(EC.PublicKey.Format.DER, Base64.decode(it))
            }.getOrNull()
        }.launchIn(backgroundScope)
    }
    ...
    override suspend fun getSharedKey(peer: Peer): AESSharedKey? {
        val secret = privateKey?.sharedSecretGenerator()?.generateSharedSecret(
            ecdhProvider.publicKeyDecoder(EC.Curve.P521)
                .decodeFromByteArray(EC.PublicKey.Format.DER, Base64.decode(peer.uuid))
        )
        secret ?: return null
    ...
    private suspend fun generateKeyPair() {
        ...
        preferencesRepository.setPublicKey(Base64.encode(keyPair.publicKey.encodeToByteArray(EC.PublicKey.Format.DER)))
        preferencesRepository.setPrivateKey(
            Base64.encode(keyPair.privateKey.encodeToByteArray(EC.PrivateKey.Format.DER))
        )
    }

    override suspend fun hashString(data: String): String {
        return Base64.encode(hashProvider.hasher().hash(data.toByteArray()))
    }
}